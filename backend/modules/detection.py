...
class CrowdDetector:
    ...
    def track_frame(self, frame: np.ndarray, conf: float | None = None, track: bool = True, draw: bool = True):
        """
    ...
    def process_frame(self, frame: np.ndarray, conf: float | None = None, track: bool = True, draw: bool = True):
        """Unified analysis pipeline wrapper. Use track=False for non-contiguous frames."""
        f, count, unique, det = self.track_frame(frame, conf, track, draw=draw)
