...
app.post('/api/send-sms', async (req, res) => {
    try {
        ...
        if (!process.env.TWILIO_SID || process.env.TWILIO_SID === 'AC0650751680da4f2ce05b47c3563bb334') {
             console.log(`[NODE-SIMULATOR] Placeholder SID detected. Simulating SMS to ${to}...`);
             return res.json({ status: "Success", message: "SMS Simulated Successfully (Demo Mode)" });
        }
        ...
        if (e.message.includes('Authenticate') || e.message.includes('not found')) {
            console.log(`[NODE-SIMULATOR] Twilio Auth failed. Falling back to simulation for ${to}...`);
            return res.json({ status: "Success", message: "SMS Simulated Successfully (Auth Fallback)" });
        }
...
app.post('/api/send-email', async (req, res) => {
    try {
        ...
        if (!process.env.EMAIL_USER || process.env.EMAIL_USER.includes('example')) {
            console.log(`[NODE-SIMULATOR] Email credentials missing or placeholder. Simulating Email to ${to}...`);
            return res.json({ status: "Success", message: "Email Simulated Successfully (Demo Mode)" });
       }
        ...
        if (e.message.includes('Invalid login') || e.message.includes('auth')) {
            console.log(`[NODE-SIMULATOR] Email Auth failed. Falling back to simulation for ${to}...`);
            return res.json({ status: "Success", message: "Email Simulated Successfully (Auth Fallback)" });
        }
        res.status(500).json({ error: e.message });
    }
});
